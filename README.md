...
Файлы **`.env`**, **`.env-aztec-agent`** и аналогичные файлы окружения содержат секреты (RPC URL, Telegram-токен и др.). **Не коммитьте их в git** — они добавлены в `.gitignore`. При клонировании репозитория не создавайте эти файлы в корне проекта без необходимости, либо убедитесь, что они не попадают в коммиты.

The files **`.env`**, **`.env-aztec-agent`** and similar environment files contain secrets (RPC URL, Telegram token, etc.). **Do not commit them to git** — they are listed in `.gitignore`.

**Проверка целостности / Integrity:** После клонирования или загрузки репозитория можно проверить скрипты: `sha256sum -c SHA256SUMS`. Файл SHA256SUMS генерируется скриптом `scripts/generate_checksums.sh`. After cloning or downloading, verify scripts with `sha256sum -c SHA256SUMS`. SHA256SUMS is generated by `scripts/generate_checksums.sh`.
...
## 📌 Последние обновления 06-02-2026
